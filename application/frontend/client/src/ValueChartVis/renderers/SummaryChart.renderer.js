...
exports.SummaryChartRenderer = void 0;
...
// columns (or rows).
let SummaryChartRenderer = /** @class */ (() => {
    let SummaryChartRenderer = class SummaryChartRenderer {
        // ========================================================================================
        // 									Constructor
        // ========================================================================================
        /*
        ...
        constructor(rendererService, renderEventsService, sortAlternativesInteraction) {
            ...
            */
            ...
            this.calculateUserScoreDimensionTwo = (d, i) => {
                var userObjectiveWeight = d.user.getWeightMap().getObjectiveWeight(d.objective.getId());
                var score = d.user.getScoreFunctionMap().getObjectiveScoreFunction(d.objective.getId()).getScore(d.value);
            ...
            this.calculateNormalizedTotalScore = (d) => {
                var scoreFunction = d.user.getScoreFunctionMap().getObjectiveScoreFunction(d.objective.getId());
            ...
            this.calculateAverageScore = (d) => {
                var totalScore = 0;
                d.userScores.forEach((userScore) => {
        ...
        */
        createSummaryChart(u) {
            ...
            this.chart = u.el.append('g')
                .classed(definitions_1.SummaryChartDefinitions.CHART, true);
            // Create the rectangle which acts as an outline for the chart.
            this.outline = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.OUTLINE_CONTAINER, true)
                .append('rect')
                .classed(definitions_1.SummaryChartDefinitions.OUTLINE, true)
            ...
            this.rowsContainer = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.ROWS_CONTAINER, true);
            this.averageLinesContainer = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.AVERAGE_LINES_CONTAINER, true);
            this.scoreTotalsContainer = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.SCORE_TOTAL_CONTAINER, true);
            this.utilityAxisContainer = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.UTILITY_AXIS_CONTAINER, true)
                .classed('utility-axis', true);
            this.alternativeBoxesContainer = this.chart.append('g')
                .classed(definitions_1.SummaryChartDefinitions.ALTERNATIVE_BOXES_CONTAINER, true);
        ...
        */
        createSummaryChartRows(u, rowsContainer, boxesContainer, scoreTotalsContainer) {
            ...
            var updateRows = rowsContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.ROW)
                .data(u.rowData);
            ...
            updateRows.enter().append('g') // Add row containers for data elements that have no matching container.
                .classed(definitions_1.SummaryChartDefinitions.ROW, true);
            ...
            this.rows = rowsContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.ROW); // Update the row field.
            var alternatives = u.valueChart.getAlternatives();
            var updateSubContainers = scoreTotalsContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.SCORE_TOTAL_SUBCONTAINER)
            ...
            updateSubContainers.enter().append('g') // Add score total sub-containers for alternatives that have no matching sub-container.
                .classed(definitions_1.SummaryChartDefinitions.SCORE_TOTAL_SUBCONTAINER, true);
            this.scoreTotalsSubContainers = scoreTotalsContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.SCORE_TOTAL_SUBCONTAINER); // Update the sub-container field.
            var updateScoreTotals = this.scoreTotalsSubContainers.selectAll('.' + definitions_1.SummaryChartDefinitions.SCORE_TOTAL)
            ...
            updateScoreTotals.enter().append('text') // Add score totals for users that do not have matching score totals.
                .classed(definitions_1.SummaryChartDefinitions.SCORE_TOTAL, true);
            this.scoreTotals = this.scoreTotalsSubContainers.selectAll('.' + definitions_1.SummaryChartDefinitions.SCORE_TOTAL); // Update the score totals field.
            var updateAverageLines = this.averageLinesContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.AVERAGE_LINE)
                .data((u.rowData[0].cells));
            updateAverageLines.exit().remove();
            updateAverageLines.enter().append('line')
                .classed(definitions_1.SummaryChartDefinitions.AVERAGE_LINE, true);
            this.averageLines = this.averageLinesContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.AVERAGE_LINE);
            var updateAlternativeBoxes = boxesContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.ALTERNATIVE_BOX)
            ...
            updateAlternativeBoxes.enter().append('rect') // Add alternative boxes for alternatives that do not have a box.
                .classed(definitions_1.SummaryChartDefinitions.ALTERNATIVE_BOX, true)
                .classed(definitions_1.SummaryChartDefinitions.CHART_ALTERNATIVE, true);
            this.alternativeBoxes = boxesContainer.selectAll('.' + definitions_1.SummaryChartDefinitions.ALTERNATIVE_BOX); // Update alternative boxes field.
        ...
        */
        createSummaryChartCells(stackedBarRows) {
            ...
            var updateCells = stackedBarRows.selectAll('.' + definitions_1.SummaryChartDefinitions.CELL)
                .data((d) => { return d.cells; });
            updateCells.exit().remove();
            updateCells.enter().append('g')
                .classed(definitions_1.SummaryChartDefinitions.CELL, true)
                .classed(definitions_1.SummaryChartDefinitions.CHART_CELL, true);
            // Save all the cells as a field of the class.
            this.cells = stackedBarRows.selectAll('.' + definitions_1.SummaryChartDefinitions.CELL);
            ...
            var updateUserScores = this.cells.selectAll('.' + definitions_1.SummaryChartDefinitions.USER_SCORE)
                .data((d, i) => { return d.userScores; });
            updateUserScores.exit().remove();
            updateUserScores.enter().append('rect')
                .classed(definitions_1.SummaryChartDefinitions.USER_SCORE, true);
            // Save all the user scores bars as a field of the class.
            this.userScores = this.cells.selectAll('.' + definitions_1.SummaryChartDefinitions.USER_SCORE);
        }
        /*
            @param u - The most recent RendererUpdate message that the SummaryChartRenderer has received.
            @returns {void}
        ...
        */
        renderSummaryChart(u) {
            ...
            var cellsToUpdate = this.cells.data((d) => { return d.cells; });
            // Update the data behind the user scores.
            var userScoresToUpdate = this.userScores.data((d, i) => { return d.userScores; });
            ...
            var scoreSubContainersToUpdate = this.scoreTotalsSubContainers
                .data(() => { return (u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical) ? u.rowData[0].cells : u.rowData[u.rowData.length - 1].cells; });
        ...
        */
        renderSummaryChartRows(u, alternativeBoxes, scoreSubContainers, scoreTotals, cells, userScores, averageLines) {
            ...
            alternativeBoxes
                ...
                .attr('id', (d) => { return 'summary-' + d.getId() + '-box'; });
            ...
            averageLines.attr('transform', (d, i) => {
                return this.rendererService.generateTransformTranslation(u.viewConfig.viewOrientation, this.calculateCellCoordinateOne(d, i, u), 0);
            });
            averageLines
                .attr(u.rendererConfig.coordinateOne + '1', 0)
                .attr(u.rendererConfig.coordinateOne + '2', (d, i) => { return this.calculateCellCoordinateOne(d, 1, u); })
                .attr(u.rendererConfig.coordinateTwo + '1', (d, i) => {
                var weightTotal = u.maximumWeightMap.getWeightTotal();
                var yVal = u.rendererConfig.dimensionTwoScale(weightTotal * this.calculateAverageScore(d));
                return (yVal * ((u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical) ? -1 : 1)) +
                    ((u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical) ? u.rendererConfig.dimensionTwoSize : 0);
            })
        ...
        */
        renderScoreTotalLabels(u, scoreTotals) {
            var verticalOffset = 15;
            var horizontalOffset = 10;
            scoreTotals
                .text((d, i) => { return Math.round(100 * this.calculateNormalizedTotalScore(d)); })
                .attr(u.rendererConfig.coordinateOne, (d, i) => {
                ...
                return (u.rendererConfig.dimensionTwoScale(weightTotal * this.calculateNormalizedTotalScore(d)) *
                    ((u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical) ? -1 : 1)) +
                    ((u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical) ? (u.rendererConfig.dimensionTwoSize - verticalOffset) : verticalOffset);
            })
        ...
        */
        highlightBestUserScores(u) {
            this.scoreTotals.classed(definitions_1.SummaryChartDefinitions.BEST_SCORE, false);
            var maxUserScores = {};
            u.usersToDisplay.forEach((user) => {
                maxUserScores[user.getUsername()] = -1;
            });
            var bestTotalScoreSelections = {};
            this.scoreTotals.nodes().forEach((element) => {
                if (element.nodeName === 'text') {
                    let selection = d3.select(element);
                    let userScore = selection.datum();
                    let scoreValue = this.calculateNormalizedTotalScore(userScore);
            ...
            u.usersToDisplay.forEach((user) => {
                bestTotalScoreSelections[user.getUsername()].classed(definitions_1.SummaryChartDefinitions.BEST_SCORE, true);
            });
        }
        /*
            @param u - The most recent RendererUpdate message that the SummaryChartRenderer has received.		@returns {void}
        ...
        */
        renderSummaryChartCells(u, cells, userScores) {
            ...
            userScores.attr(u.rendererConfig.coordinateTwo, (d, i) => {
                var userObjectiveWeight = d.user.getWeightMap().getObjectiveWeight(d.objective.getId());
                var score = d.user.getScoreFunctionMap().getObjectiveScoreFunction(d.objective.getId()).getScore(d.value);
                this.summaryChartScale.domain([0, d.user.getWeightMap().getWeightTotal()]);
                if (u.viewConfig.viewOrientation === types_1.ChartOrientation.Vertical)
        ...
        */
        toggleUtilityAxis(displayScales) {
            if (displayScales) {
        ...
        */
        toggleScoreTotals(displayTotalScores) {
        ...
        */
        toggleAverageLines(displayAverageScoreLines) {
    ...
    SummaryChartRenderer = __decorate([
        core_1.Injectable(),
    ...
    return SummaryChartRenderer;
})();
exports.SummaryChartRenderer = SummaryChartRenderer;
//# sourceMappingURL=SummaryChart.renderer.js.map