...
exports.ExpandScoreFunctionInteraction = void 0;
...
*/
let ExpandScoreFunctionInteraction = /** @class */ (() => {
    let ExpandScoreFunctionInteraction = class ExpandScoreFunctionInteraction {
        // ========================================================================================
        // 									Constructor
        // ========================================================================================
        /*
        ...
        */
        constructor(chartUndoRedoService) {
            ...
            this.SCORE_FUNCTION_ROUTE = document.baseURI + 'scoreFunction/plot'; // The route that is matched to the ScoreFunctionViewer. This is the
            // route that the pop-up window will navigate to when it is opened.
            this.WINDOW_OPTIONS = 'menubar=no,location=yes,resizable=yes,scrollbars=yes,status=yes,width=620,height=600'; // The options string for the pop-up window.
            // This is defined as a field rather than a method to avoid creating a new scope.
            this.expandScoreFunction = (eventObject) => {
                var objective = d3.select(eventObject.target).datum();
        ...
        */
        toggleExpandScoreFunction(enableExpanding, scoreFunctionPlots, rendererUpdate) {
            ...
            this.clicks = Observable_1.Observable.fromEvent(scoreFunctionPlots, 'dblclick');
        ...
        */
        openPopUp(objective) {
            ...
            window.scoreFunctions = this.lastRendererUpdate.scoreFunctions;
            window.colors = this.lastRendererUpdate.colors;
            window.objectiveToPlot = objective;
            window.chartUndoRedoService = this.chartUndoRedoService;
            window.enableInteraction = this.lastRendererUpdate.interactionConfig.adjustScoreFunctions;
            window.individual = this.lastRendererUpdate.individual;
            // Open the pop-up. Note that this.popUpRef is a reference to the child window.
            this.popUpRef = window.open(this.SCORE_FUNCTION_ROUTE, 'Expanded' + objective.getId() + 'ScoreFunction', this.WINDOW_OPTIONS);
            // Save the reference to the child window.
            window.childWindows = window.childWindows || {};
            window.childWindows.scoreFunctionViewer = this.popUpRef;
        }
    };
    ExpandScoreFunctionInteraction = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [services_1.ChartUndoRedoService])
    ], ExpandScoreFunctionInteraction);
    return ExpandScoreFunctionInteraction;
})();
exports.ExpandScoreFunctionInteraction = ExpandScoreFunctionInteraction;
//# sourceMappingURL=ExpandScoreFunction.interaction.js.map