...
class WeightMap {
    ...
    */
    setInternalWeightMap(newMap) {
        this.weights = newMap;
    }
    getObjectiveWeight(objectiveId) {
        return this.weights.get(objectiveId);
    }
    getNormalizedObjectiveWeight(objectiveId) {
        return this.weights.get(objectiveId) / this.weightTotal;
    }
    setObjectiveWeight(objectiveId, weight) {
        var priorWeight = this.weights.get(objectiveId);
    ...
    removeObjectiveWeight(objectiveId) {
        var priorWeight = this.weights.get(objectiveId);
        if (priorWeight !== undefined) {
            this.weightTotal -= priorWeight;
        }
    ...
    */
    recalculateWeightTotal() {
        var weightIterator = this.weights.values();
        var weight = weightIterator.next();
        var sumOfWeights = 0;
    ...
    */
    getObjectiveWeights(orderedObjectives) {
        var orderedWeights = [];
        for (var i = 0; i < orderedObjectives.length; i++) {
    ...
    */
    getNormalizedWeights(orderedObjectives) {
    ...
    */
    normalize() {
        var objectiveIterator = this.weights.keys();
        var key = objectiveIterator.next();
    ...
    */
    getMemento() {
        ...
        var weightMapCopy = new WeightMap();
        ...
        this.getInternalWeightMap().forEach((value, key) => {
...
//# sourceMappingURL=WeightMap.js.map